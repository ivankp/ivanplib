...
opt_match_type make_opt_match_impl(T&& x, opt_match_tag<char>) {
  if (x=='\0') throw error("NULL character used as short option matcher");
  if (x=='-') throw error("all dashes matches only as context option");
...
#endif
  if (x[0]=='\0') throw error("NULL string used as option matcher");
  if (t==short_opt) {
    if (x[2]!='\0') throw error(
