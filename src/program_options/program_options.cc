...
bool program_options::parse(int argc, char ** argv,
  ...
  for (int i=1; i<argc; ++i) {
    ...
    if (!opt || (opt->is_multi() && opt->count)) {
      for (auto& m : matchers[opt_type]) {
        if ((*m.first)(arg)) { // match
          ...
          if (opt->is_switch()) {
            if (val) {
              if (opt_type!=context_opt) throw error(
    ...
    throw error("unexpected option ",arg);
    next_arg: ;
  } // end arg loop
  if (opt) {
    if (!opt->count) opt->as_switch();
    else if (!last_was_val) throw error("dangling option " + opt->name);
    opt = nullptr;
  }
...
void arg_parser_impl_bool(const char* arg, bool& var) {
  ...
  else throw error('\"',arg,"\" cannot be interpreted as bool");
}
